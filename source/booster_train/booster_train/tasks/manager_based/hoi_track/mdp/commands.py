...
class MotionCommand(CommandTerm):
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if self.cfg.play:
            # Deal the clips out across envs so one play/export run shows every
            # motion from its first frame. Do not invoke the training-only
            # adaptive sampler in this mode.
