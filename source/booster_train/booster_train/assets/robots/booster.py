...
# Same robot with a collision pad on each palm -- the sim equivalent of bolting a rubber pad onto the end
# effector. Generated by scripts/make_bighands_urdf.py; identical links, joints, inertias and meshes, so a
# policy trained on the stock robot loads unchanged and only the contact geometry differs.
BOOSTER_K1_BIGHANDS_CFG = BOOSTER_K1_CFG.replace(
    spawn=BOOSTER_K1_CFG.spawn.replace(
        asset_path=f"{BOOSTER_ASSETS_DIR}/robots/K1/K1_22dof_bighands.urdf",
    )
)
